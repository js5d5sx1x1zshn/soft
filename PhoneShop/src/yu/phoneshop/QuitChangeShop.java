...
public class QuitChangeShop extends Activity{
	/** volley对象*/
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.quit_change_activity);
...
//				textViewOrderid.setText(text);
				break;
			case 1:
				Toast.makeText(QuitChangeShop.this, "请检查网络连接", Toast.LENGTH_SHORT).show();
				break;
			case 2:
				Toast.makeText(QuitChangeShop.this, "取消申请已提交", Toast.LENGTH_SHORT).show();
				finish();
				break;
			case 3:
				Toast.makeText(QuitChangeShop.this, "取消失败", Toast.LENGTH_SHORT).show();
				break;
			case 4:
				Toast.makeText(QuitChangeShop.this, "取消申诉申请已提交到后台", Toast.LENGTH_SHORT).show();
				break;
			case 5:
				Toast.makeText(QuitChangeShop.this, "申请待审核", Toast.LENGTH_SHORT).show();
				break;
			case 6:
				Toast.makeText(QuitChangeShop.this, "提交失败", Toast.LENGTH_SHORT).show();
				break;
			default:
	...
	private void cancelOrder(){
		requestQueue.add(new StringRequest(CommonUtil.SERVER_URL1+"CancelOrder?orderid="+orders.getOrderId()+"", new Listener<String>() {
			@Override
			public void onResponse(String response) {
	...
	private void submitApply(){
		requestQueue.add(new StringRequest(CommonUtil.SERVER_URL1+"SubmitApply?orderid="+orders.getOrderId()+"&status=1", new Listener<String>() {
			@Override
			public void onResponse(String response) {
				Log.e("response", "response"+response);
				if(response.equals("0")){
